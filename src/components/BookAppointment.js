...
export default function BookAppointment(){
       const doctorId = useSelector(state=>state.bookAppointmentIdreducer.doctorid)
        ...
        function handleClick(event){
          ...
          axios.post('https://wecare2-data.onrender.com/bookings',state)
           .then((response)=>{
              ...
              setSuccessMessage(`Your Appointment is schedule successfully with appointment id:
              ${response.data.id}`)
            })
           .catch((error)=>{
              console.log(error)
                ...
                <br></br>
                   <div className="container w-50">
                        <div className="card bg-black ">
                                <div className="card-header text-center text-light">Proceed with your Appointment</div>
                                      <div className="card-body">
                                                    <label className="form-label text-light">Date of Appointment</label>
                                                    <input type="date" className="form-control" 
                                                    name="appointmentDate" value={state.appointmentDate}
                                                    onChange={(event)=>handleChange(event)}/>
                                                <br></br>
                                               <p className="text-light">Prefered Slot</p>

                                               <p><select name="slot" value={state.slot} onChange={(event)=>{handleChange(event)}}>
                                                ...
                                                <button className="btn btn-primary" onClick={(event)=>handleClick(event)}>Book your Appointment</button>
                                                {(successMessage) ? <div style={{color:"white"}}>{successMessage}</div>:null}
                                                {(errorMessage) ? <div style={{color:"red"}}>{errorMessage}</div>:null}

                                                <button className="btn btn-primary" onClick={(event)=>{handleBack(event)}}>GoBack</button>
                                         </div>
                            </div>
                    </div>                    
