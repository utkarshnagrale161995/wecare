...
import { bookAppointmentId } from "../actions/action";
...
export default function UserHome(){
    ...
    function getPatientDetails(){ 
       ...
       function handleBook(event,id){
        event.preventDefault()
        dispatch(bookAppointmentId(id))
    ...
    return(
        <>
                    ...
                    <button className="btn btn-secondary" onClick={(event)=>handleAppointments(event)}>MyAppointments</button>
                    
                    <div><h1>List of Doctors</h1></div>
                   <div>
                   {
                   details.map((detail) => { 
                    return (
                         <div className="card" style={{ margin: "20px" }}> 
                                 <h4 className="card-header">Dr. {detail.name}</h4> 
                                 <div className="card-body" style={{ padding: "10px" }}> 
                                        <p>Doctor Id : {detail.id}</p> 
                                        <p>Mobile No :{detail.mobileNumber}</p> 
                                        <p>Speciality :{detail.speciality}</p>
                                        <button className="btn btn-secondary" onClick={(event)=>handleBook(event,detail.id)}>Book an Appointment</button> 
                                 </div> 
                         </div> 
                        ); 
                        })
